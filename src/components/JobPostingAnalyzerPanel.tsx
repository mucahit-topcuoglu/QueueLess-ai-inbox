...
export function JobPostingAnalyzerPanel() {
  ...
  async function handleAnalyze() {
    ...
    try {
      ...
      setError(requestError instanceof Error ? requestError.message : "Recruitment analizi tamamlanamadı.");
    } finally {
  ...
  return (
    <InfoCard title="İş ilanı analizi" description="İlan linki veya manuel ilan metni ile PDF CV belgelerini Gemini API üzerinden analiz edin.">
      ...
      >
        {isAnalyzing ? "Gemini analiz ediyor" : "İlan ve CV'leri Analiz Et"}
      </button>
          ...
          <div className="mt-3 space-y-2">
            {result.data.candidates.map((candidate) => (
              <div key={candidate.fileName} className="rounded-md border border-emerald-200 bg-white p-3">
                ...
                <p className="mt-1">Aksiyon: {candidate.recommendedAction}</p>
                <p className="mt-1">{candidate.aiSummary}</p>
                {candidate.missingFields.length > 0 ? (
                  <div className="mt-3 rounded-md bg-amber-50 p-3 text-amber-950">
                    <p className="font-black">Eksikler</p>
                    <ul className="mt-1 list-disc pl-5">
                      {candidate.missingFields.map((field) => <li key={field}>{field}</li>)}
                    </ul>
                  </div>
                ) : null}
                ...
                <div className="mt-3 rounded-md bg-slate-50 p-3">
                  ...
                  <pre className="mt-2 whitespace-pre-wrap font-sans text-xs leading-5 text-slate-700">{candidate.generatedReplyDraft || "Taslak olusturulamadi; manuel kontrol gerekir."}</pre>
                </div>
              </div>
            ))}
          </div>
