...
export async function POST(request: Request) {
  try {
    ...
    if (!jobTextResult.text) {
      return NextResponse.json({ ok: false, error: "İş ilanı linki okunamadı, manuel ilan metni girin." }, { status: 400 });
    }
...
async function resolveJobText(jobUrl: string, manualJobText: string): Promise<{ text: string; warning?: string }> {
  ...
  try {
    ...
    if (url.hostname.includes("linkedin.com") && url.pathname.includes("/jobs/collections")) {
      return {
        text: "",
        warning: "LinkedIn öneri/collection linkleri giriş gerektirebilir. Manuel ilan metni girin."
      };
    }
